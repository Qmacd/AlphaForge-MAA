...
def main(
    ...
    for seed in seeds:
        if isinstance(seeds,str):
            seeds = eval(seeds)
        assert isinstance(seeds,list)   
        path = os.path.join(os.getcwd(), "out", f"{save_name}_{instruments}_{train_end}_{seed}", "z_bld_zoo_final.pkl")
        tensor_save_path = os.path.join(os.getcwd(), "out", f"{save_name}_{instruments}_{train_end}_{seed}")
