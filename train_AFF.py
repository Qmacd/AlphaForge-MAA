...
from gan.network.generater import train_network_generator, train_multi_agent_generator
import gc
...
def get_metric(zoo_blds,device,corr_thresh=0.5,metric_target='ic'):
    ...
    def get_score(fct,tgt):
        ...
        if n_blds > 0 and score > 0.:
            ...
            all_matrix = torch.cat([existed,ret[None]],dim=0) # (n_blds+1,n_days)
...
def main(
    ...
    for seed in seeds:
        ...
        class cfg:
            ...
            g_hidden = 128
            g_lr = 1e-3
            use_multi_agent = True # 是否使用多智能体模式
            N = 3 # 多智能体模式下，智能体数量
            distill = False
        ...
        while len(zoo_blds)<cfg.num_factors:
            ...
            if cfg.use_multi_agent:
                # 设置多智能体数量
                N = cfg.N  # 可根据需要自定义智能体个数

                # TODO: 采用不同的生成器和判别器进行对抗训练
                # 初始化多个生成器
                netGs = [NetG_CLS(
                    n_chars=SIZE_ACTION,
                    latent_size=cfg.potential_size,
                    seq_len=cfg.max_len,
                    hidden=cfg.g_hidden
                ).to(cfg.device) for _ in range(N)]

                # 初始化多个判别器
                netDs = [torch.nn.Sequential(
                    torch.nn.Linear(cfg.max_len * SIZE_ACTION, 128),
                    torch.nn.ReLU(),
                ...
                # 构造判别器监督矩阵
                init_weight_matrix = np.ones((N, N + 1)) / (N + 1)  # 初始阶段平均监督
                final_weight_matrix = np.eye(N, N + 1)  # 后期自监督为主

                # 调用多智能体生成器训练函数，返回 Builders
                blds_in_train = train_multi_agent_generator(
                    netGs=netGs,
                    netM=netM,
                    netP=netP,
                    netDs=netDs,
                    ...
                    metric=metric,
                    lr=cfg.g_lr,
                    init_weight_matrix=init_weight_matrix,
